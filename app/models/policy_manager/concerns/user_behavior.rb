module PolicyManager::Concerns::UserBehavior
  ...
  included do
    ...
    PolicyManager::Config.rules.each do |rule|
      ...
      if rule.validates_on
        ...
        validate :"check_#{rule_name}", on: rule.validates_on, if: lambda { |o|
        ...
        define_method :"check_#{rule_name}" do
          return unless send(rule_name).blank? && needs_policy_confirmation_for?(rule.name)
      ...
      define_method :"#{rule_name}=" do |val = true|
        ...
        instance_variable_set("@#{rule_name}", val)
        ut = user_terms.new
  ...
  def portability_collection_for(rule, page = nil)
    send(rule.collection).paginate(page: page, per_page: rule.per)
  end
  ...
  def pending_policies
    ...
    PolicyManager::Config.rules.select do |c|
      needs_policy_confirmation_for?(c.name)
    end
  end

  def pending_blocking_policies
    PolicyManager::Config.rules.select do |c|
      c.blocking && needs_policy_confirmation_for?(c.name)
    end
  end
  ...
  def policy_user_term_on(term)
    ...
    user_terms.where(term_id: term.id).first
  end

  def handle_policy_for(term)
    user_terms.where(term_id: term).first_or_initialize do |member|
      member.term_id = term.id
    end
  end

  def can_request_portability?
    portability_requests.select { |p| p.pending? || p.progress? }.blank?
  end
  ...
  def accept_policy_from(name)
    ...
    user_term = handle_policy_for(term)
