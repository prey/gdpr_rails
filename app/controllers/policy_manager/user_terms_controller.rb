...
module PolicyManager
  class UserTermsController < ApplicationController
    ...
    skip_before_action :user_authenticated?, only: %i[show accept reject blocking_terms]
    before_action :set_user_term, only: %i[accept reject show edit update destroy]

    # GET /user_terms/1
    def show
      @user_term = if current_user.present?
                     current_user.handle_policy_for(@term)
                   else
                     UserTerm.new(term: @term, user: nil,
                                  state: cookies["policy_rule_#{@term.rule.name}"])
                   end
    end
    ...
    def blocking_terms
      respond_to do |format|
        format.html {}
        format.json do
          render json: PolicyManager::Config.rules
                                            .select { |p| p.blocking }
                                            .map(&:name)
        end
      end
        ...
        format.json do
          if @user_term.present? && @user_term.errors.any?
            render json: { url: root_url,
                           notice: "hey there are some errors! #{@user_term.errors.full_messages.join}" }
          else
            render json: {
