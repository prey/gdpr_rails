...
module PolicyManager
  class ExporterHandler
    ...
    def handle_render_for(rule)
      render_member(rule) if rule.member

      return unless rule.collection
    ...
    def render_member(rule)
      ...
      base_dir = base_path.join(rule.name)
      ...
      return unless rule.json_template.present?

      JsonExporterView.new(
        assigns: { member: o },
    ...
    def render_collection(rule)
      ...
      (1..o.total_pages).to_a.each do |i|
        ...
        next unless rule.json_template.present?
    ...
    def render_index
      resource_path = base_path.join('index.html')
